  ...
  for sentence in sentences {
    let tree = cetz.canvas({
      ...
      for (i, token) in sentence.tokens.enumerate() {
        ...
        content((x, 0.0), text(size: 24pt, [#token.form]), name: safe-name)
        
        let y-offset = -0.68
        if show-lemma and token.lemma != "_" {
          content((x, y-offset), text(size: 20pt, fill: rgb("555555"), token.lemma))
          y-offset -= 0.42
        }
        if show-upos and token.upos != "_" {
          content((x, y-offset), text(size: 20pt, fill: rgb("0055aa"), token.upos))
          y-offset -= 0.42
        }
        if show-xpos and token.xpos != "_" {
          content((x, y-offset), text(size: 20pt, fill: rgb("aa5500"), token.xpos))
          y-offset -= 0.42
        }
      }
        ...
        for root in sentence.roots {
          ...
          content(
            ...
            box(
              ...
              text(size: 20pt, weight: "bold", fill: root-color, root.label)
            )
          )
        }
      }
      ...
      for arc in sentence.arcs {
        ...
        content(
          ...
          box(
            fill: white, inset: (x: 2pt, y: 1pt), radius: 2pt,
            text(size: 20pt, fill: if is-highlighted { stroke-color } else { luma(30) }, arc.label)
          )
        )
      }
    })
